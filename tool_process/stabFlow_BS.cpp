...
int main(int argc, char** argv){
	...
	String inBB(argv[12]); // estimated bbox region
	int cropBB = atoi(argv[13]); // how much more to crop
    setDevice(device_id);
    ...
    // 3. main algo
	Mat prev_grey, grey, grey_, frame, flow_x, flow_y, imgX, imgY, im0, im1, im0_bs;
    ...
    Size sz_bs = im0_bs.size(); // for homography warping

    Size sz_bb = stabCrop.size(); // estimated after bs removed
    Size szNew;
    if(sz_bb.width>sz_bb.height){
        szNew = Size((int)((float)sz_bb.width*256.0/(float)sz_bb.height),256);
    }else{
        szNew = Size(256,(int)((float)sz_bb.height*256.0/(float)sz_bb.width));
    }
    ...
    for(int i=f_st-1;i<f_lt;i+=step){
        ...
        if(i==f_md-1){
            ...
            // warp image
            warpPerspective(im0_bs, im1, H0, sz_bs);
            frame = im1(stabCrop);
        }
		if(i == f_st-1) {
			grey_.create(sz_bb, CV_8UC1);
