...
int main(int argc, char** argv){
	...
	String inBB(argv[6]); // estimated bbox region
	int cropBB = atoi(argv[7]); // how much more to crop
    ...
    cornerRead.close();
    Rect stabCrop(corner[2]+cropBB, corner[0]+cropBB, (corner[5]-corner[2]-corner[3]-2*cropBB+1), (corner[4]-corner[0]-corner[1]-2*cropBB+1));
    //Rect stabCrop(corner[0]+cropBB, corner[2]+cropBB, (corner[4]-corner[0]-corner[1]-2*cropBB+1), (corner[5]-corner[2]-corner[3]-2*cropBB+1));
    //cout<<corner[0]+cropBB<<"_"<<corner[2]+cropBB<<"_"<<(corner[4]-corner[0]-corner[1]-2*cropBB+1)<<"_"<<(corner[5]-corner[2]-corner[3]-2*cropBB+1)<<endl;
    // image i/o
	Mat im0,imOut;
    ...
    //cout<<im0.rows<<"_"<<im0.cols<<endl;
    imOut = im0(stabCrop);
    //cout<<imOut.rows<<"_"<<imOut.cols<<endl;
    cvtColor(imOut, im0_gray, CV_BGR2GRAY);
    capture.release();
    ...
    for(int i=f_st-1;i<f_lt;i++){
        ...
        //cout<<imOut.rows<<"_"<<imOut.cols<<endl;
        if(i!=f_md-1){
            cvtColor(imOut, im1_gray, CV_BGR2GRAY);
