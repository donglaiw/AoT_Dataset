        ...
        if(i!=f_md-1){
            ...
            if(points[0].size()>0){
                ...
                for(int j=0;j<points[1].size();j++){
                    // mask out points
                    //cout<<points[0][j].x<<"_"<<points[0][j].y<<"_"<<im0_gray.cols<<"_"<<im0_gray.rows<<endl;
                    if(points[0][j].x<=m_w || points[0][j].x>=im0_gray.cols-m_w || points[0][j].y<=m_h || points[0][j].y>=im0_gray.rows-m_h ){ 
                        Point2f dif = points[0][j]-points[2][j];
                        //cout<<points[0][j]<<abs(dif.x)+abs(dif.y)<<"_"<<fb_thres<<endl;
